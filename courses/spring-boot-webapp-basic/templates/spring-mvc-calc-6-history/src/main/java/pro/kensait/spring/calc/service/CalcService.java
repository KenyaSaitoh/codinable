...
public class CalcService {
    ...
    // サービスメソッド：足し算の実行
    public int add(BigDecimal param1, BigDecimal param2) {
        ...
        // テーブルに計算結果を保存する
        CalcResult calc = new CalcResult(param1, param2, CalcType.ADD, result,
                LocalDateTime.now());
        dao.save(calc);
    ...
    // サービスメソッド：引き算の実行
    public int subtract(BigDecimal param1, BigDecimal param2) {
        ...
        // テーブルに計算結果を保存する
        CalcResult calc = new CalcResult(param1, param2, CalcType.SUBTRACT, result,
                LocalDateTime.now());
        dao.save(calc);
    ...
    // サービスメソッド：掛け算の実行
    public int multiply(BigDecimal param1, BigDecimal param2)
        ...
        // テーブルに計算結果を保存する
        CalcResult calc = new CalcResult(param1, param2, CalcType.MULTIPLY, result,
                LocalDateTime.now());
        dao.save(calc);

        // 計算結果が極度オーバーしていないかチェックし、発生した場合は例外をスローする
        BigDecimal limit = new BigDecimal(config.getCalcResultLimit());
        if (0 < calc.getResult().compareTo(limit)) {
    ...
    // サービスメソッド：割り算の実行
    public int divide(BigDecimal param1, BigDecimal param2)
        ...
        // テーブルに計算結果を保存する
        CalcResult calc = new CalcResult(param1, param2, CalcType.DIVIDE, result,
                LocalDateTime.now());
        dao.save(calc);
