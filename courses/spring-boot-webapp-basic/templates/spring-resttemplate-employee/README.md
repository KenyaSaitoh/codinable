# spring-resttemplate-employee — RestTemplate による同期 HTTP 通信
...
| 場所 | 中身 | 動かし方 |
|---|---|---|
| ルート（`src/`） | 確認用の社員 API（Spring Boot、ポート 8086、DB 不要でメモリ内に社員 6 件） | 「実行」ボタン（`gradle:bootRun`） |
| `client/` | RestTemplate で API を呼ぶ Java コンソール（`Main_Employee`）。処理が終わると終了する | ターミナル |
...
1. 実行対象で `gradle:bootRun` を選び、「実行」を押す（API が `http://127.0.0.1:8086` で待ち受ける）
...
接続先は、第 1 引数（`../gradlew.bat run --args=http://127.0.0.1:8086`）→ 環境変数 `EMPLOYEE_API_URL`
→ 既定値 `http://localhost:8086` の順で決まります。
