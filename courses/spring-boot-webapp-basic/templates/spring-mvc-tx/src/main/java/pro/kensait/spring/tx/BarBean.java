...
/*
 * 1つめのXAデータソース（dataSource1）を更新するクラス ローカルトランザクション版（spring-mvc-tx）ではREQUIRES_NEWとの対比を行ったが、 ここでは2つのデータソースを同一のグローバルトランザクションに参加させるため、 REQUIREDを指定して呼び出し元（FooBean）のトランザクションに参加する
 */
@Service
