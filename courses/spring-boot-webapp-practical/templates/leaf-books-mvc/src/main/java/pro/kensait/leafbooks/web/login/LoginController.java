...
import static pro.kensait.leafbooks.util.Util.*;
...
/*
 * ログインのコントローラーを担うクラス （ログイン画面はCognitoのマネージドログインが提供するため、エラー画面への遷移のみを担当する）
 */
@Controller
public class LoginController {
    ...
    public String processLogin(
        ...
        try {
            customer = customerApiClient.queryByEmail(loginParam.email());
        ...
        if (savedRequest != null) {
            ...
            try {
                ...
                // * null値を返してメソッドを抜ける
                // * ここで処理を抜けないと、レスポンスがコミットされた状態で別のリダイレクトが呼ばれてしまうため
                return null;
