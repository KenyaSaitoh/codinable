...
public class EmployeeService {
    ...
    // MicrometerのMeterRegistry（Actuatorによって自動的にBean登録される）
    @Autowired
    private MeterRegistry meterRegistry;

    // コンストラクタ
    public EmployeeService(EmployeeRepository employeeRepository,
            DepartmentRepository departmentRepository, MeterRegistry meterRegistry) {
    ...
    public EmployeeTO getEmployee(Integer employeeId) {
        ...
        // カスタムメトリクス：社員取得の呼び出し回数をカウントアップする
        // （/actuator/metrics/employee.get.count で確認できる）
        meterRegistry.counter("employee.get.count").increment();
