...
public class EmployeeService {
    ...
    // MicrometerのObservationRegistry（自動的にBean登録される）
    @Autowired
    ...
    public EmployeeTO getEmployee(Integer employeeId) {
        ...
        // Observation APIによる計装：処理をObservationで包むことで、
        // 1つの計装からメトリクス（employee.getタイマー）とトレース（スパン）の
        // 両方が生成される（メトリクスは/actuator/metrics/employee.getで確認できる）
        return Observation.createNotStarted("employee.get", observationRegistry)
                .lowCardinalityKeyValue("employee.type", "regular")
                .observe(() -> {
                    // 主キー検索を実行し、存在しない場合は
                    // EmployeeNotFoundExceptionを送出する
    ...
    // サービスメソッド：全社員を取得する
    // @Observedアノテーションによる計装：ObservedAspectの働きにより、
    // Observation APIと同じくメトリクスとトレースの両方が生成
    @Observed(name = "employee.get-all", contextualName = "employee#get-all")
